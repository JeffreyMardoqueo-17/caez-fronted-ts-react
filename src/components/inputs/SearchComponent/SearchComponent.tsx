    ...
    return (
        <div className="relative" ref={containerRef}>
            <input
                ...
                placeholder="Nombre"
            />
            {loading && <p className="absolute left-0 right-0 mt-2 dark:bg-darkTheme-background p-2 border dark:border-darkTheme-background rounded shadow bg-lightTheme-primary">Cargando...</p>}
            {showResults && (
                <ul className="absolute left-0 right-0 mt-2 bg-lightTheme-primary dark:bg-darkTheme-formulario dark:text-darkTheme-gray border rounded dark:border-darkTheme-background shadow-md max-h-60 overflow-y-auto z-50  ">
                    {results.map((result, index) => (
                        <li
                            key={index}
                            className={`p-2 cursor-pointer ${highlightedIndex === index ? 'dark:bg-darkTheme-background' : ''} hover:bg-lightTheme-background`}
