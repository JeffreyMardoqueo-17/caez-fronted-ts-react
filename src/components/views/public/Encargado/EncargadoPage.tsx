...
const EncargadoPage = () => {
    ...
    const filtrar=(terminoBusqueda: string)=>{
        var resultadosBusqueda=tablaEncargados.filter((elemento)=>{
    ...
    // Transformar los datos para las filas de la tabla
    const tableRows = tablaEncargados.map(encargado => [
        encargado.Id,
        encargado.Nombre,
        encargado.Apellido,
        encargado.Role, // Asumiendo que 'Cargo' se refiere a 'Role'
        encargado.Telefono,
        encargado.TelEmergencia,
        encargado.Correo,
        encargado.TipoDocumento,
        encargado.NumDocumento, // Asumiendo que '#' se refiere a 'NumDocumento'
        encargado.Administrador,
        new Date(encargado.FechaRegistro).toLocaleDateString() // Formatear la fecha
    ]);
